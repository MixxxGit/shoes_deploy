...
find_domain_and_certs() {
    ...
    # Find the most recently modified directory, which we assume is the active domain
    ...
    # Common cert names for acme.sh and certbot
...
configure_firewall() {
    ...
    if command -v ufw &> /dev/null; then
        if ! sudo ufw status | grep -q "Status: active"; then warn "UFW is inactive. Skipping firewall configuration."; return; fi
        for port in "${PORTS_TO_OPEN[@]}"; do if ! sudo ufw status | grep -qw "$port"; then info "Opening port $port on UFW..."; sudo ufw allow "$port/tcp"; sudo ufw allow "$port/udp"; else info "Port $port is already open on UFW."; fi; done
        success "Step complete: UFW configuration finished."
    elif command -v firewall-cmd &> /dev/null; then
        if ! sudo firewall-cmd --state &> /dev/null; then warn "firewalld is not running. Skipping firewall configuration."; return; fi
        reload_needed=false
        for port in "${PORTS_TO_OPEN[@]}"; do
            if ! sudo firewall-cmd --query-port="$port/tcp" --permanent &> /dev/null; then info "Opening port $port/tcp..."; sudo firewall-cmd --permanent --add-port="$port/tcp"; reload_needed=true; else info "Port $port/tcp is already open."; fi
            if ! sudo firewall-cmd --query-port="$port/udp" --permanent &> /dev/null; then info "Opening port $port/udp..."; sudo firewall-cmd --permanent --add-port="$port/udp"; reload_needed=true; else info "Port $port/udp is already open."; fi
        done
...
generate_client_link() {
    local template_name=$1; info "Step: Generating client configuration..."; local link=""; local tag="${DOMAIN}-shoes"
    case "$template_name" in
